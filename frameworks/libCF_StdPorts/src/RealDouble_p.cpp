...
#include <iostream>
#include "../include/RealDouble_p.h"
...
StandardInterfaces_i::RealDouble_p::~RealDouble_p() {
#ifdef _DEBUG
    std::cout << "this is Class RealDouble_p destructor" << std::endl;
#endif
}

CORBA::Object_ptr
StandardInterfaces_i::RealDouble_p::getPort(
    const char * _portName) {
    if (strcmp(_portName, portName.c_str()) == 0) {
        std::cout << "[StandardInterfaces_i::RealDouble_p] find port:  " << _portName <<
                  std::endl;
...
void
StandardInterfaces_i::RealDouble_p::getData(
    PortTypes::DoubleSequence *& data) {
    std::cout << "[StandardInterfaces_i::RealDouble_p::getData] entering in getData"
              << std::endl;
#ifdef __SDS_OS_VXWORKS__
    semTake(sem_data_ready, WAIT_FOREVER);
#elif defined __SDS_OS_LINUX__
    sds_lock("realdouble_data_ready");
#endif
    data = & data_in;
    std::cout << "[StandardInterfaces_i::RealDouble_p::getData] leaving getData" <<
              std::endl;
}

void
StandardInterfaces_i::RealDouble_p::bufferEmptied() {
#ifdef __SDS_OS_VXWORKS__
    semGive(sem_ready_for_input);
#elif defined __SDS_OS_LINUX__
    sds_unlock("realdouble_ready_for_input");
#endif
};
...
void
RealDouble::ProvidesPort::pushPacket(
    const PortTypes::DoubleSequence & data) {
    std::cout << "[RealDouble::providesPort::pushPacket] entering in pushPacket" <<
              std::endl;
#ifdef __SDS_OS_VXWORKS__
    semTake(base->sem_ready_for_input, WAIT_FOREVER);
#elif defined __SDS_OS_LINUX__
    sds_lock("realdouble_ready_for_input");
#endif
    std::cout << "[RealDouble::providesPort] have get sem" << std::endl;
...
#elif defined __SDS_OS_LINUX__
    sds_unlock("realdouble_data_ready");
#endif
    std::cout << "[RealDouble::providesPort] leaving pushPacket and give sem" <<
              std::endl;
}