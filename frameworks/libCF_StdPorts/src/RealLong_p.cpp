...
StandardInterfaces_i::RealLong_p::~RealLong_p() {
#ifdef _DEBUG
    std::cout << "this is Class RealLong_p destructor" << std::endl;
#endif
}

CORBA::Object_ptr
StandardInterfaces_i::RealLong_p::getPort(
    const char * _portName) {
    if (strcmp(_portName, portName.c_str()) == 0) {
        std::cout << "[StandardInterfaces_i::RealLong_p] find port:  " << _portName <<
                  std::endl;
...
void
StandardInterfaces_i::RealLong_p::getData(
    JTRS::LongSequence *& data) {
#ifdef __SDS_OS_VXWORKS__
    semTake(sem_data_ready, WAIT_FOREVER);
#elif defined __SDS_OS_LINUX__
    sds_lock("reallong_data_ready");
#endif
    data = & data_in;
}

void
StandardInterfaces_i::RealLong_p::bufferEmptied() {
#ifdef __SDS_OS_VXWORKS__
    semGive(sem_ready_for_input);
#elif defined __SDS_OS_LINUX__
    sds_unlock("reallong_ready_for_input");
#endif
};
...
void
RealLong::ProvidesPort::pushPacket(
    const JTRS::LongSequence & data) {
    std::cout << "[RealLong::providesPort::pushPacket] entering in pushPacket" <<
              std::endl;
#ifdef __SDS_OS_VXWORKS__
    semTake(base->sem_ready_for_input, WAIT_FOREVER);
#elif defined __SDS_OS_LINUX__
    sds_lock("reallong_ready_for_input");
#endif
    std::cout << "[RealLong::providesPort::pushPacket] have get sem" << std::endl;
...
#elif defined __SDS_OS_LINUX__
    sds_unlock("reallong_data_ready");
#endif
    std::cout <<
              "[RealLong::providesPort::pushPacket] leaving pushPacket and give sem" <<
              std::endl;
}